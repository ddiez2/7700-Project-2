    ...
    def __init__(self, filepath, sp=None, max_seq_length=512):
        ...
        with open(filepath, "r", encoding="utf-8") as f:
            for line in f:
                ...
                tokens = data["tokens"]
                # Convert tokens to integers if necessary.
...
def collate_fn(batch, pad_token=0):
    """
    Pads a batch of (input, target) pairs so that all sequences have the same length.
    """
...
class LSTMLM(nn.Module):
    """
    A language model based on LSTM for next-token prediction.
    ...
    Weight tying is employed by sharing weights between the embedding and output layer.
    
    The forward() method either returns logits (for training) or samples the next token
    at the final time step using temperature scaling (for generation).
    
    The prompt() method uses autoregressive generation for text completion.
    """
    def __init__(self, vocab_size, d_model=256, num_layers=2, dropout=0.2, eos_token=3, max_seq_length=512):
        ...
        # LSTM: using batch_first=True for consistency.
        self.lstm = nn.LSTM(
            input_size=d_model, 
            hidden_size=d_model, 
        ...
        # Fully connected output layer; weight tying with the embedding.
    ...
    def forward(self, x, temperature=1.0, sample=False):
        ...
        Args:
            x: Tensor (batch, seq_len) of token IDs.
            temperature: Sampling temperature.
            sample: If True, sample the next token from the final time step.
        
        Returns:
            - If sample is False: logits with shape (batch, seq_len, vocab_size).
            - If sample is True: predicted next token IDs.
        """
        embedded = self.embedding(x) * math.sqrt(self.d_model)
        embedded = self.dropout(embedded)
        outputs, _ = self.lstm(embedded)  # outputs shape: (batch, seq_len, d_model)
        outputs = self.dropout(outputs)
        logits = self.fc(outputs)
        
        if sample:
            # Sample from the logits of the last time step.
    ...
    def prompt(self, text, sp, device, max_gen_length=None, temperature=1.0):
        """
        Autoregressively generates text based on an input prompt.
        
        Args:
            text: Input prompt as a string.
...
def evaluate_bleu(model, test_filepath, sp, device, max_gen_length=None, temperature=1.0, eval_percentage=100):
    ...
    Only predicts a single token after each prompt (next-token prediction).
    
    Args:
        model: The language model.
        test_filepath: Path to the JSONL test file.
        sp: SentencePieceProcessor.
        device: Torch device.
        max_gen_length: Maximum sequence length (unused here).
        temperature: Sampling temperature.
        eval_percentage: Percentage of test data used.
    """
    references = []
    hypotheses = []
...
def main():
    ...
    # Clean up validation losses for plotting.
    val_losses_cleaned = []
